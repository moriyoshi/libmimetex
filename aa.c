...
int aalowpass(raster *rp, intbyte *bytemap, int grayscale)
{
    ...
    weights[4] = centerwt;
    /*adjacent pts*/
    weights[1] = weights[3] = weights[5] = weights[7] = adjacentwt;
    /* tot is center plus neighbors */
    totwts = centerwt + 4 * (1 + adjacentwt);
    /* ------------------------------------------------------------
...
int aapnm(raster *rp, intbyte *bytemap, int grayscale)
{
    ...
    if (isresetparams) {         /* wts[], etc taken from params */
        ...
        wts[4] = centerwt;
        /* and adjacent points */
        wts[1] = wts[3] = wts[5] = wts[7] = adjacentwt;
        /* and corner points */
        wts[0] = wts[2] = wts[6] = wts[8] = cornerwt;
        /* sum wts */
...
int aapnmlookup(raster *rp, intbyte *bytemap, int grayscale)
{
    ...
    for (irow = 0; irow < height; irow++) {
        for (icol = 0; icol < width; icol++) {
            ...
            gridnum = aagridnum(rp, irow, icol);
            ...
            patternum = aapatternnum(gridnum);
            /* some internal error */
                ...
                int gridnum, int grayscale)
{
    ...
    int turn1 = 0, turn2 = 0, aafollowline();
    /* ------------------------------------------------------------
...
int aasupsamp(raster *rp, raster **aa, int sf, int grayscale)
{
    ...
    /*indexes, i=width j=height*/
    int irp = 0, jrp = 0, iaa = 0, jaa = 0, iwt = 0, jwt = 0;
    /* raster for supersampled image */
    raster  *aap = NULL, *new_raster();
    /* get weight matrix applied to rp */
    raster  *aaweights();
    /* aaweights() resultant matrix */
    static  raster *aawts = NULL;
    ...
    ------------------------------------------------------------ */
    /* --- check args --- */
    /* no ptr for return output arg */
    if (aa == NULL) goto end_of_job;
    /* init null ptr for error return */
    *aa = NULL;
    if (rp == NULL               /* no ptr to input arg */
            ||   sf < 1             /* invalid shrink factor */
            /* invalid grayscale */
            ||   grayscale < 2) goto end_of_job;
    ...
    heightrem = rpheight % sf;
    /* ...by shrinkfactor */
    widthrem  = rpwidth % sf;
    /* ss height */
    aaheight  = 1 + (rpheight + sf - (heightrem + 1)) / sf;
    /* ss width */
    aawidth   = 1 + (rpwidth + sf - (widthrem + 1)) / sf;
    if (msgfp != NULL && msglevel >= 999) {
    ...
    if (isrescalemax) {          /* flag set to rescale maxaaval */
        ...
        for (jaa = 0; jaa < aaheight; jaa++) /* height */
            for (iaa = 0; iaa < aawidth; iaa++) { /* width */
                /* un-rescaled value */
                int aafrac, aaval = getpixel(aap, jaa, iaa);
                /*multiply by scale factor*/
                aaval = (int)(0.5 + ((double)aaval) * scalef);
                /* fraction of blackness */
                aafrac = (100 * aaval) / (grayscale - 1);
        ...
        if (ncolors > 1)            /* and if not a "blank" raster */
            if (colors[ncolors-1] > 0) {       /*and at least one pixel non-white*/
                for (igray = 1; igray < ncolors; igray++) { /*gamma correct each colors[]*/
                    ...
                    /* apply gamma correction */
                    dgray = pow(dgray, (1.0 / gammacorrection));
