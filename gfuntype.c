 ...
 * ======================================================================= */
/* --- entry point --- */
static int	parsecorner (char *line, int *row, int *col)
{
    /* --------------------------------------------------------------------------
    Allocations and Declarations
    -------------------------------------------------------------------------- */
 ...
 * ======================================================================= */
/* --- entry point --- */
static int	findnextchar (mimetex_ctx *mctx, FILE *fp, int *location)
{
    /* --------------------------------------------------------------------------
    Allocations and Declarations
    -------------------------------------------------------------------------- */
    static	char keyword[99]="beginning of char "; /*signals start of next char*/
    char	*line;	/* read next line from fp */
 ...
 * ======================================================================= */
/* --- entry point --- */
static int	bitcmp (unsigned char *bs1, unsigned char *bs2, int n)
{
    /* --------------------------------------------------------------------------
    Allocations and Declarations
    -------------------------------------------------------------------------- */
 ...
 * ======================================================================= */
/* --- entry point --- */
static int	rasterizechar (FILE *fp, raster *image)
{
    /* --------------------------------------------------------------------------
    Allocations and Declarations
    -------------------------------------------------------------------------- */
    ...
    if (gformat == 0		/* choose optimal/smallest respresentation */
        ...
        for (iformat=2; iformat<=3; iformat++) {   /* 2=8-bit packing, 3=4-bit */
            ...
            if (gfbitcount > 0) {	/* have a final count */
                ...
                pixcount++;
            }		/* don't bump pixcount within macro */
        ...
        nbytes2=npixcounts[0];
        nbytes3= (1+npixcounts[1]) /2; /* #bytes for count */
 ...
 * ======================================================================= */
/* --- entry point --- */
static chardef	*getnextchar (mimetex_ctx *mctx, FILE *fp)
{
    /* --------------------------------------------------------------------------
    Allocations and Declarations
    -------------------------------------------------------------------------- */
    chardef	*nextchar= (chardef *) NULL; /*ptr returned to caller*/
    int	charnum, location = 0; /* get header line for next char */
    int	parsestat= (-999); /* get col,row from ".<--" line */
    /* --------------------------------------------------------------------------
    initialization
    -------------------------------------------------------------------------- */
    while (parsestat == (-999)) {		/* flush entirely blank characters */
        ...
        if (nextchar == (chardef *) NULL)	/* haven't allocated chardef yet */
            if ( (nextchar=new_chardef(mctx))	/* allocate a new chardef */
...
error:
    if (nextchar != (chardef *) NULL)	/* have an allocated chardef */
        delete_chardef (mctx, nextchar);		/* so deallocate it */
 ...
 * ======================================================================= */
/* --- entry point --- */
static char *getcharname (char *fontname, int charnum)
{
    /* --------------------------------------------------------------------------
    Allocations and Declarations
    -------------------------------------------------------------------------- */
...
int	main (int argc, char *argv[])
{
    ...
    while (argc > ++argnum)	/* check for flags and filenames */
        if (*argv[argnum] == '-') {	/* got some '-' flag */
            ...
            case 'g':
                ...
                break;
            case 'u':
                isnoname = atoi (argv[argnum]);
                break;
            case 'm':
                mctx.msglevel = atoi (argv[argnum]);
                break;
            case 'n':
                strcpy (fontname,argv[argnum]);
                break;
            } /* --- end-of-switch() --- */
        } /* --- end-of-if(*argv[]=='-') --- */
    ...
    mctx.msgfp = (outarg>0? stdout : stderr); /* use stdout or stderr */
    ...
    if (mctx.msglevel >= 9)		/* verbose output requested */
        fprintf (mctx.msgfp,"gfuntype> infile=%s outfile=%s, fontname=%s format=%d.%d\n",
    ...
    while ( (nextchar=getnextchar (&mctx, infp)) != NULL) { /* get each char in file */
        ...
        if (mctx.msglevel >= 19)			/* if a bit more verbose */
            type_raster (&mctx, & (nextchar->image),mctx.msgfp); /*display ascii image of raster*/
